...
class MoviePlayer:
    ...
    def play(self):
        ...
        eve = self.eve
        eve.cmd_playvideo(eve.OPT_MEDIAFIFO | eve.OPT_FULLSCREEN | eve.OPT_NOTEAR)
        eve.cmd_nop()
        eve.flush()
    ...
    def service(self):
        """ Send movie TODO XXX chunk by chunk"""
        eve = self.eve
        ...
        while fullness < (self.mf_size - chunk_size):
            chunk = self.file_handler.read(chunk_size)
            if not chunk:
                return
            eve.wr(self.mf_base + self.write_pointer, chunk)
