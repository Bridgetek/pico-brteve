...
SS_START               = 0
def ADDR(X, Y)      : return (X["addr"] - Y["addr"])
def H_CENTER(x)     : return (int)((SCREEN_WIDTH - (x)) * 0.5)     
def V_CENTER(x)     : return (int)((SCREEN_HEIGHT - (x)) * 0.5)    

def Y_CENTER(IMG, y): return ((y) - IMG['height'] * 0.5)         
def X_CENTER(IMG, x): return ((x) - IMG['width']* 0.5)          
