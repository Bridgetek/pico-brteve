...
_BAUDRATE_HS_921600 = const(0x00010353)
...
class VC0706:
    ...
    def clean(self):
        ...
        # Clear buffer to ensure the end of a string can be found.
        self._send_command(_GEN_VERSION, b"\x01")
        self._read_response(self._buffer, 1)
    ...
    def Write_to(self):
        return
        # 0x56+serial number+0x31+Data-length+device type+the data num ready to write+register address(2 bytes)+data.
        # 0x56+0x00+0x31+0x06+0x01+0x02+0x18+0x00+0x11+0x22
        # Begin from chip register address 0x1800, write 2 bytes data,0x11,0x22.
        
        Dtype_Chip_register=0x01
        Dtype_Sensor_register=0x02
        Dtype_CCIR656_register=0x03
        Dtype_I2C_EEPROM=0x04
        Dtype_SPI_EEPROM=0x05
        Dtype_SPI_Flash=0x06
        ...
        if not self._run_command(_SET_PORT, bytes([dlen, dtype, AddrByte1, AddrByte2, datanum, data]), 7):
            raise RuntimeError("Failed to enable_HSUART!")
            
    def enable_HSUART(self):
        # 0x56+serial number+0x31+Data-length+device type+the data num ready to write+register address(2 bytes)+data.
        # 0x56+0x00+0x31+0x06+0x01+0x02+0x18+0x00+0x11+0x22
        # Begin from chip register address 0x1800, write 2 bytes data,0x11,0x22.
        ...
        if not self._run_command(_SET_PORT, bytes([dlen, dtype,Byte1, Byte2, Byte3, Byte4]), 5):
            print("Failed to enable_HSUART 1!")
            
        elif not self._run_command(_SET_PORT, bytes([dlen, dtype,Byte1, Byte2, Byte3, Byte4]), 6):
            print("Failed to enable_HSUART 2!")
            
        elif not self._run_command(_SET_PORT, bytes([dlen, dtype,Byte1, Byte2, Byte3, Byte4]), 7):
            print("Failed to enable_HSUART 3!")
    ...
    def compress(self, ratio):
        ...
        vc_comp_ratio = ((ratio - 13) * 4) + 53 # math lol

        return self._run_command(
            _WRITE_DATA, bytes([0x05, 0x01, 0x01, 0x12, 0x04, 0x00]), 5
        )
